...
<body class="">
    ...
    @foreach ($errors->all() as $error)
        ...
        @endforeach
    @endif
    <form action="{{ route('admin.updateTruck',$truck->tid) }}" method="POST" class="m-5 p-5 border shadow-lg">
        @csrf
        <div class="row">
            ...
            <div class="col-12">
                <div class="form-floating">
                    <select name="hauler_id" id="" class="form-control my-2" required>
                        <option value="{{ $truck->hauler_id }}">{{ $truck->hauler_name}} - {{$truck->hauler_number}}</option>
