<!DOCTYPE html>
    ...
    <body class="">
        ...
        <form action="{{ route('admin.updateDriver',$id) }}" method="POST" class="m-5 p-5 border shadow-lg">
            @csrf
            <div class="row">
                ...
                <div class="col-12">
                    <div class="form-floating">
                        <input type="text" name="address" class="form-control my-2" id="address" value="{{ $user->uaddress }}" placeholder="Choose Unique address">
                ...
                <div class="col-12">
                    <div class="form-floating">
                       <select name="hauler_id" id="hauler" class="form-control">
                        <option value="{{ $user->haul_id }}">{{$user->haulerName}}</option>
                        @foreach ($haulers as $hauler )
                            <option value="{{ $hauler->id }}">{{$hauler->name}} | {{$hauler->shipp_number}}</option>
                        @endforeach

                       </select>
                        <label for="hauler" class="form-label">Expiration Date</label>
                    </div>
                </div>
                <div class="col-12">
