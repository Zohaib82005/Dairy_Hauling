...
<body class="">
    <h5 class="text-center bg-primary p-1 text-white">Define Routes</h5>
    @if ($errors->any())
        @foreach ($errors->all() as $error)
            <div class="alert alert-danger">{{$error}}</div>
        @endforeach
    @endif
    <form action="{{ route('admin.insert.route') }}" method="POST" class="m-5 p-5 border shadow-lg">
        @csrf
        <div class="row">

            <div class="col-12">
                <div class="form-floating">
                    <input type="text" name="route_number" required class="form-control my-2" id="truck_id" placeholder="Enter Truck ID">
            ...
            <div class="col-12">
                <div class="form-floating">
                    <select name="destination_plant" id="dp" class="form-control my-2" required>
                        <option selected disabled>Select Destination Plant</option>
            ...
            <div class="col-12">
                <div class="form-floating text-center">
                    <button type="submit" class=" btn btn-primary m-2 p-2 px-5" >Add Route</button>
                </div>
            </div>
